...
// Copié depuis catalogue-standard.js — à garder identique au fichier source
...
async function main() {
  ...
  for (const etabDoc of etabsSnap.docs) {
    ...
    if (!produitsSnap.empty) continue; // déjà un catalogue, on ne touche pas

    console.log(`⚠ ${etabDoc.id} : 0 produit, génération du catalogue de démarrage...`);
    let batch = db.batch();
    ...
    await batch.commit();
    console.log(`✔ ${etabDoc.id} : ${CATALOGUE_STANDARD.length} produits créés (stock 10 chacun)`);
  }
  console.log("\nTerminé.");
}
